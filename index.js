...
bot.on('callback_query', async (query) => {
    ...
    try{
        ...
        if(query.data === 'new offer' && state.telegram){
            ...
            bot.sendMessage(telegramId, `
            <b>На данный момент проект нуждается в поддержке 🫶</b>/n/n
            🤝 <b>Часть оплаты аренды серверов осуществляется из личного кармана сообщества. Хоть проект острое нуждается в средствах, мы не стали повышать цены на тарифы</b>/n/n
            ✔️ Наша задача сделать доступный комфортный VPN. Было вложено много усилий и времени, мы планируем и дальше понижать цены за подписки./n/n
            🫶 Вы можете помочь, оформив более дорогую подписку, заплатив больше, или пригласив друзей. У нашего VPN 
            нет ограничений по устройствам, скорости и торрентам. Геолокация страны со свободой от цензуры в интернете, мы гарантруем 
            вам полную приватность и безопасность./n/n
            ✔️ Для тех, кто содержит группы или сообщества мы можем предложить <b><u>до года бесплатного подключения VPN за 1 пост.</u></b> Контакт — @Albert_zero2/n/n
            <b>Выберите подписку, или воспользуйтесь опцией "Какую подписку мне выбрать" 👇</b>/n/n
            `.format(), state.options);
            return
        }
    }
